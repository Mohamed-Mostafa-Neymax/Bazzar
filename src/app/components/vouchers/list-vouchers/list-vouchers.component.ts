...
export class ListVouchersComponent implements OnInit {
  ...
  onListVouchers(){
    ...
    this.globalService.allVouchers().subscribe( shopsRes => {
      
      this.vouchesArr = shopsRes['data']
      console.log('vouchesArr', this.vouchesArr);
    } );
  }
  ...
  onEditVoucher(voucher){
    ...
    dialogRef.afterClosed().subscribe( res => this.onListVouchers() );
  }

  onDeleteVoucher(voucher_id){
    this.spinner.show();
    this.globalService.deleteVouchers(voucher_id).subscribe( deleteRes => {
      this.spinner.hide();
      Swal.fire(
        'نجاح',
