...
@Component({
  selector: 'app-remove-tag-shop',
  templateUrl: './remove-tag-shop.component.html',
  styleUrls: ['./remove-tag-shop.component.scss']
})
export class RemoveTagShopComponent implements OnInit {
  
  removeTagForm: FormGroup;
  ...
  onSubmit() {
    this.spinner.show();
    this.globalService.removeTagShop(this.removeTagForm.value.shop_id[0]['id'], this.removeTagForm.value.home_tag_id[0]['id'])
      .subscribe( 
        addFilterShopRes => {
          this.spinner.hide();
          Swal.fire(
            'نجاح',
            'تم حذف العلامة للمتجر بنجاح',
            'success'
          );
